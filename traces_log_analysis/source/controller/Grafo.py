...
from controller.Vertice import Vertice
from controller.Aresta import Aresta
...
class Grafo:
    ...
    def nova_Aresta(self, origem, destino, peso):  # Metodo recebe dois identificadores
    ...
    def busca_Adjacente(self, u):  # Metodo recebe um vertice
        for i in range(len(self.lista_Arestas)):
            ...
            if (u.getId() == origem.getId()) and (destino.getVisitado() == False):
                destino.setVisitado(True)  # Para que nao retorn o mesmo vertice seguidas veses
                return destino
        else:
            return None
    ...
    def visita(self, u):
        ...
        v = self.busca_Adjacente(u)  # retorna apenas nao visitado ou nulo
        while v is not None:
    ...
    def inicializa_Fonte(self, fonte):  # Funcao usado no BFS e Dijkstra Metodo recebe um Objeto
        for v in self.lista_Vertices:
    ...
    def Breadth_first_search(self, identificador):
        ...
        while 0 != len(lista):
            u = lista[0]
            v = self.busca_Adjacente(u)  # retorna adjacente nao visitado
            if v is None:
    ...
    def Bellman_Ford(self, origem):
        ...
        for w in self.lista_Arestas:
            ...
            v = w.getDestino()
            if u.getEstimativa() + w.getPeso() < v.getEstimativa():
                return False  # Nao existe ciclo negativo
            else:
    ...
    def Minimum_spanning_tree(self, origem):  # Prim
        ...
        while len(lista) != 0:
            ...
            if v is None:
                for i in lista:  # como o vetice u marcou seus adj como visitado nenhum outro vertice visitara
                    i.setVisitado(
                        False)  # esse vertice entao preciso marcar como nao visitado pra bucar os adj de outro vertice
    ...
    def is_Cyclic(self):
        if (len(self.lista_Arestas) > len(self.lista_Vertices) - 1):
            print("Grafo Ciclico por N Aresta ")
#            print("Grafo Ciclico por N Aresta :",len(self.lista_Arestas),  "> N Vertices: " , len(self.lista_Vertices)
        else:
    ...
    def Strong_component_algorithm(self):
        ...
        self.lista_Vertices.sort(key=lambda u: u.output, reverse=True)  # ordena a lista em ralacao a vertice.output
        for w in self.lista_Arestas:
            print(w)
    ...
    def Articulation(self):
        ...
        for u in self.lista_Vertices:
            if self.eh_Ponto(u):
                art.append(u.getId())
        print("Pontos de Articulacao", art)
        ####################################################################